...
service.get("/logout", function(req, res) {
	...
	mongo.findOne(userCollectionName, criteria, function(err, record) {
		if(err) { return res.jsonp(req.soajs.buildResponse({"code": 404, "msg": config.errors[404]})); }

		if(!record) { return res.jsonp(req.soajs.buildResponse({"code": 404, "msg": config.errors[404]})); }
...
service.get("/forgotPassword", function(req, res) {
	var mongo = new Mongo(req.soajs.meta.tenantDB(req.soajs.registry.tenantMetaDB, config.serviceName, req.soajs.tenant.code));
	mongo.findOne(userCollectionName, {'username': req.soajs.inputmaskData['username'], 'email': req.soajs.inputmaskData['email'], 'status': 'active'}, function(err, userRecord) {

		if(err) { return res.jsonp(req.soajs.buildResponse({"code": 406, "msg": config.errors[406]})); }

		if(!userRecord) { return res.jsonp(req.soajs.buildResponse({"code": 406, "msg": config.errors[406]})); }
		...
		mongo.update(tokenCollectionName, {'username': userRecord.username, 'service': 'forgotPassword', 'status': 'active'}, {'$set': {'status': 'invalid'}}, function(err) {
			if(err) { return res.jsonp(req.soajs.buildResponse({"code": 407, "msg": config.errors[407]})); }
...
service.post("/resetPassword", function(req, res) {
	...
	var mongo = new Mongo(req.soajs.meta.tenantDB(req.soajs.registry.tenantMetaDB, config.serviceName, req.soajs.tenant.code));
	//get token
	mongo.findOne(tokenCollectionName, {'token': req.soajs.inputmaskData['token'], 'service': {'$in': ['forgotPassword', 'addUser']}, status: 'active'}, function(err, tokenRecord) {
		if(err || !tokenRecord) { return res.jsonp(req.soajs.buildResponse({"code": 406, "msg": config.errors[406]})); }
		...
		mongo.findOne(userCollectionName, {'username': tokenRecord.username}, function(error, userRecord) {
			if(error || !userRecord) { return res.jsonp(req.soajs.buildResponse({"code": 406, "msg": config.errors[406]})); }
...
service.post("/join", function(req, res) {
	...
	mongo.findOne(userCollectionName, {$or: [{'username': req.soajs.inputmaskData['username']}, {'email': req.soajs.inputmaskData['email']}]}, function(err, record) {
		...
		//user exits
		if(record) { return res.jsonp(req.soajs.buildResponse({"code": 402, "msg": config.errors[402]})); }
...
service.get("/join/validate", function(req, res) {
	...
	//get token
	mongo.findOne(tokenCollectionName, {'token': req.soajs.inputmaskData['token'], 'service': 'join', status: 'active'}, function(err, tokenRecord) {
		if(err || !tokenRecord) { return res.jsonp(req.soajs.buildResponse({"code": 406, "msg": config.errors[406]})); }
		...
		//get user record
		mongo.findOne(userCollectionName, {'username': tokenRecord.username, 'status': 'pendingJoin'}, function(error, userRecord) {
			if(error || !userRecord) { return res.jsonp(req.soajs.buildResponse({"code": 406, "msg": config.errors[406]})); }
...
service.get("/account/getUser", function(req, res) {
	var mongo = new Mongo(req.soajs.meta.tenantDB(req.soajs.registry.tenantMetaDB, config.serviceName, req.soajs.tenant.code));
	mongo.findOne(userCollectionName, {'username': req.soajs.inputmaskData['username'], 'status': 'active'}, function(err, record) {
		if(err || !record) { return res.jsonp(req.soajs.buildResponse({"code": 405, "msg": config.errors[405]})); }
...
service.post("/account/changePassword", function(req, res) {
	...
	mongo.findOne(userCollectionName, {'_id': userId}, function(err, userRecord) {
		if(err || !userRecord) { return res.jsonp(req.soajs.buildResponse({"code": 405, "msg": config.errors[405]})); }
...
service.post("/account/changeEmail", function(req, res) {
	...
	mongo.findOne(userCollectionName, {'_id': userId}, function(err, userRecord) {
		...
		if(userRecord.email === req.soajs.inputmaskData['email']) { return res.jsonp(req.soajs.buildResponse({"code": 412, "msg": config.errors[412]})); }
		...
		mongo.update(tokenCollectionName, {'username': userRecord.username, 'service': 'changeEmail', 'status': 'active'}, {'$set': {'status': 'invalid'}}, function(err) {
			if(err) { return res.jsonp(req.soajs.buildResponse({"code": 407, "msg": config.errors[407]})); }
...
service.get("/changeEmail/validate", function(req, res) {
	//check if user account is there
	var mongo = new Mongo(req.soajs.meta.tenantDB(req.soajs.registry.tenantMetaDB, config.serviceName, req.soajs.tenant.code));
	mongo.findOne(tokenCollectionName, {'token': req.soajs.inputmaskData['token'], 'service': 'changeEmail', status: 'active'}, function(err, tokenRecord) {
		if(err || !tokenRecord) { return res.jsonp(req.soajs.buildResponse({"code": 406, "msg": config.errors[406]})); }
...
service.get("/admin/listUsers", function(req, res) {
	...
	mongo.find(userCollectionName, {}, {}, function(err, userRecords) {
		if(err || !userRecords) { return res.jsonp(req.soajs.buildResponse({"code": 405, "msg": config.errors[405]})); }

		//if no records return empty array
		if(userRecords.length === 0) { return res.jsonp(req.soajs.buildResponse(null, [])); }
...
service.post("/admin/addUser", function(req, res) {
	...
	mongo.findOne(userCollectionName, {'username': req.soajs.inputmaskData['username']}, function(err, record) {
		if(err) { return res.jsonp(req.soajs.buildResponse({"code": 414, "msg": config.errors[414]})); }
...
function updateUserRecord(req, complete, cb) {
	...
	mongo.findOne(userCollectionName, {'_id': userId}, function(err, userRecord) {
		if(err || !userRecord) { return cb({"code": 405, "msg": config.errors[405]}); }

		//check if username is taken by another account
		mongo.count(userCollectionName, {'_id': {'$ne': userId}, 'username': req.soajs.inputmaskData['username']}, function(err, count) {
			if(err) { return cb({"code": 407, "msg": config.errors[407]}); }

			//if count > 0 then this username is taken by another account, return error
			if(count > 0) { return cb({"code": 410, "msg": config.errors[410]}); }
...
service.post("/admin/group/edit", function(req, res) {
	...
	mongo.update(groupsCollectionName, {'_id': groupId}, s, {'upsert': false, 'safe': true}, function(error) {
		if(error) { return res.jsonp(req.soajs.buildResponse({"code": 418, "msg": config.errors[418]})); }
		return res.jsonp(req.soajs.buildResponse(null, true));
	});
});
