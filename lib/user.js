...
function saveRecord(soajs, combo, pin, index, cb) {
	...
	libProduct.model.saveEntry(soajs, combo, function (err) {
		if (err) {
			if (err.message.indexOf("config.allowedTenants.tenant.pin.code") !== -1) {
				obj.tenant.pin = {};
				saveRecord(soajs, combo,  pin, index, cb);
			} else {
				return cb(err);
			}
		} else {
		...
		checkSAASSettings(() => {
			...
			libProduct.model.findEntry(req.soajs, combo, function (err, record) {
				...
				utils.checkIfError(req, cb, data, true, function () {
					...
					libProduct.model.validateId(req.soajs, req.soajs.tenant.id.toString(), function (e, tId) {
						if (e) {
			...
			if (userRecord.config.allowedTenants.length > 0) {
				userRecord.config.allowedTenants.forEach((oneTenant) => {
					if (oneTenant.tenant && oneTenant.tenant.id
						&& oneTenant.tenant.id === req.soajs.tenant.id.toString()) {
						found = true;
					}
				});
			}
