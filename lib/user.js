...
var libProduct = {
	...
	"inviteUsers": function (req, cb) {
		libProduct.model.initConnection(req.soajs);
		if (!req.soajs.inputmaskData.usernames && !req.soajs.inputmaskData.emails) {
			return cb({"code": 428, "msg": req.soajs.config.errors[428]});
		}
		let combo = {
			collection: userCollectionName,
		...
		if (req.soajs.inputmaskData.usernames) {
			combo.condition.username = {
				"$in": req.soajs.inputmaskData.usernames
			};
		}
		if (req.soajs.inputmaskData.emails) {
			combo.condition.email = {
				"$in": req.soajs.inputmaskData.emails
			};
		}
		let updatedRecords = [];
		//validate the user record
		libProduct.model.findEntries(req.soajs, combo, function (err, userRecords) {
			...
			async.eachSeries(userRecords, function(userRecord, callback) {
				if (!req.soajs.tenant.locked && userRecord.locked) {
					return callback();
				}
