...
function saveRecord(soajs, combo, pin, index, mainTenant, cb) {
	...
	if (pin) {
		if (pin.code) {
			...
			try {
				if (mainTenant) {
                    combo.record.tenant.pin.code = makePin(pinCode);
    ...
    if (combo.record._id) {
		...
		combo.extraOptions = {"upsert" : false};
	}
	else {
        combo.condition = {};
        combo.updatedFields = combo.record;
    ...
    //NOTE chnage this to update with upsert true
    libProduct.model.updateEntry(soajs, combo, function (err) {
        if (err) {
