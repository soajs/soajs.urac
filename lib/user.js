...
var libProduct = {
	...
	"inviteUser": function (req, cb) {
		...
		if (req.soajs.inputmaskData.owner) {
			combo.condition.owner = req.soajs.inputmaskData.owner;
		}
	...
	"unInviteUsers": function (req, cb) {
		...
		if (req.soajs.inputmaskData.owner) {
			combo.condition.owner = {
				"$in": req.soajs.inputmaskData.owner
			};
		}
		combo.updatedFields = {
			"$pull": {
	...
	"addEditPinCode": function (req, cb) {
		...
		libProduct.model.findEntry(req.soajs, combo, function (err, userRecord) {
			...
			if (index === -1) {
				...
				if (req.soajs.inputmaskData.groups){
					allowedTenants[index].tenant.groups = req.soajs.inputmaskData.groups;
				}
			}
