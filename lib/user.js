...
function saveRecord(soajs, combo, pin, index, cb) {
	if (pin) {
		if (pin.code) {
			combo.record.config.allowedTenants[index].tenant.pin.code = makePin(soajs.config.pinCode.length);
			...
			if (index === -1) {
				...
				if (req.soajs.inputmaskData.pin) {
					if (req.soajs.inputmaskData.pin.code){
						allowedTenants[index].tenant.pin.code= makePin(req.soajs.config.pinCode.length);
					}
