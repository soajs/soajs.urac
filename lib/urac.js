...
var libProduct = {
	...
	"guest": {
		"customLogin": function (req, res) {
			...
			userRecord.profile.social[mode] = {
				"id": user.profile.id
			};
			...
			var c = {};
			c['profile.social.' + mode + '.id'] = user.profile.id;
			condition["$or"].push(c);
			...
			libProduct.model.findEntry(req.soajs, combo, function (err, record) {
				if (err) {
					...
					return res.jsonp(req.soajs.buildResponse({"code": 400, "msg": req.soajs.config.errors[400]}));
				}
				
				if (record) {
					// update record
					if (!record.profile.social) {
						record.profile.social = {};
					}
					if (!record.profile.social[mode]) {
						record.profile.social[mode] = {};
					}
					record.profile.social[mode].id = user.profile.id;
					record.profile.social[mode].accessToken = user.accessToken;
					
					if(user.refreshToken){ // first time application authorized
						record.profile.social[mode].refreshToken = user.refreshToken;
					}
					...
					userRecord.profile.social[mode].accessToken = user.accessToken;
					
					if(user.refreshToken){ // first time application authorized
						userRecord.profile.social[mode].refreshToken = user.refreshToken;
					}
			...
			libProduct.model.findEntry(req.soajs, combo, function (err, tokenRecord) {
				...
				checkIfError(req, res, data, true, function () {
					...
					if (new Date(tokenRecord.expires).getTime() < new Date().getTime()) {
						...
						return res.jsonp(req.soajs.buildResponse({
							"code": 406,
							"msg": req.soajs.config.errors[406]
						}));
					}
			...
			libProduct.model.validateId(req.soajs, req.soajs.inputmaskData['uId'], function (err, userId) {
				...
				libProduct.model.findEntry(req.soajs, combo, function (err, userRecord) {
					if (err || !userRecord) {
						...
						return res.jsonp(req.soajs.buildResponse({
							"code": 405,
							"msg": req.soajs.config.errors[405]
						}));
					}
	...
	"admin": {
		...
		"user": {
			...
			"listUsers": function (req, res) {
				...
				var fields = {'password': 0, 'config': 0};
				var combo = {
					collection: userCollectionName,
					condition: condition,
					fields: fields,
			...
			"getUser": function (req, res) {
				...
				libProduct.model.validateId(req.soajs, req.soajs.inputmaskData['uId'], function (err, userId) {
					if (err) {
						...
						return res.jsonp(req.soajs.buildResponse({
							"code": 411,
							"msg": req.soajs.config.errors[411]
						}));
					}
					var combo = {
						collection: userCollectionName,
